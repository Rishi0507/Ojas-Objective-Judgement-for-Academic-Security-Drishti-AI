...
unusual for this part of this room," it never creates or suppresses one. A
z-score says the pixels moved unusually, not that a person cheated, and
...
*where their hands are*. Head turns, signalling, and reaching toward a
...
5fps". That arithmetic was simply wrong: two frames span one interval,
...
itself measured. That separation is deliberate, so the two kinds of claim
...
covers only ordering (`seq` + `prev_hash` + statement hash, proving nothing
...
reassemble. Purely a presentation layer: it reads fields the Go backend
already emits, recomputes nothing, and mutates no event.
